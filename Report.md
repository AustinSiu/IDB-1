...
Inside the `artists` dictionary contains every artist known to the database and each item in the dictionary is another dictionary with the artists' `id` as a key value. For the each individual `artist` model there are eight attributes that are stored; `name`, `id`, `genre`, `img`, `active`, and links to `tours`, `albums`, and `songs`.
    ...
    * `id` is the id we use to represent unique artists which are also used as the key value for the each artists' dictionary.
    * `genre` is a list of the string containing the genres related to the artist.
    * `img` is a string of the URL of an image related to the artist.
    * `active`  is a dictionary with two keys `start` and `end`. `start` and `end` are integers that represent the year the artists got started and ended; if they are still active, the end would be the current year.
...
We began implementing React in Phase III of the project. Two key tools that we have been using to apply React to our project are Webpack and Babel.js. Webpack is nothing more complicated than a code bundler. It takes our code, transforms and bundles it, then returns a brand new version of our code for us to put to production. What’s neat about Webpack is that its purpose is to listen to any transformations that need to be made to our code and then it will transform it and output a bundle file full of those changes. The file name that contains our webpack configurations is named webpack.config.js and it is located in the root directory of our project. When webpack runs, the transformed code could be referenced in a file located in our dist folder. In the folder we’ll have two files, index_bundle.js and index.html. index_bundle.js is the result of our entry code that we ran through our loaders, and index.html is a copy of our original index.html file from our app folder.
...
Now let’s talk about the actual React components themselves. Our components are comprised of our models for the project: Artists, Albums, Songs, and Tours. Each of these components has a render method, which describes the user interface. In index.js, we instructed React to take our four components and render it to the element with an ID of ‘app’. We are only using this once due to the parent/child relations of React. This theory rules that when you render the most parent component, it will render all of its child components as well. Components are great this way because they're both modular and reusable. You can take one component used in one area of an application and reuse it in other areas without having to duplicate any code. This increases efficiency in development. Each component decides how it should be rendered, and each has its own internal logic.
...
In a nutshell, we are using React as one piece of a larger app that will do all the processing of the data. Our data is pulled from a database, then saved back to the database as the interface is being used. React is a great way to develop a website for many reasons. One is that the reusable benefit of the components allows our app to look and feel consistent. It also makes development for our app easier.
...
For our project, we created four Javascript files to help build the search implementation: Search.js, SearchBox.js, SearchResults.js, and SearchResultsGrid.js. The parent component in this case is Search.js. Here is where we declared the main search function and pass in the filter parameters. Searches are case-insensitive and include substrings. The `searchString` is passed on as props because it will then be passed on to SearchResults. Search results are displayed and categorized by model, allowing the user to tab between the four models for matching results. Each result is stored in the grid which is displayed through SearchResultsGrid.js. In the case of a search with multiple words, we first display "and" results, which are all of the results that contain all of the search terms. We then display the "or" results, which include any model that matches at least one term in the search. Finally, below the model, we show the matching context of the search with the term highlighted, so that the user can see why each result appeared. All of the results are contextualized and clickable. Also, our results are dense and have at least 10 results per page. If our results are more than 10 results, the user may navigate to the next page through our pagination feature. Our goal was to mimic the ever-so-great Google search engine.

### Adding/Deleting/Editing: Flask and Backend

Our site uses React for the most part: upon visiting banddb.me, the server sends index.html to client alongside a JS file to render it. When new data is needed, JS use jQuery to visit API created by Flask-Restless to help to render. Flask provides two things in this process: 1. Handle visits to the index page (‘/’) and throw index.html and JS file to the client. 2. Provide APIs at banddb.me

For adding/deleting/editing, there are two approaches that can be taken. The first one is to enable DELETE and POST on APIs generated. We think this way exposes our database to Internet attacks, programmed editing or deleting or some unknown robots visiting. The second way is handle adding/deleting/editing through Python Flask. With new endpoints created by Flask, we are able to limit writing permissions to specific pages. On that page, we can use robot checking (Google reCAPTCHA) to rule out robots and programs. It also hinders users from abusing their right of editing. In principle a log-in system can also be built to help manage to edit on our database, but it requires more complicated design.

Now Python Flask generates 3 endpoints: `‘/’, ‘/edit/<string:_type>/<int:_id>’, and ‘add/<string:_type>;` it also handles all other URL paths back to index page.

For editing, the endpoint is `/edit/<string:_type>/<int:_id>`, which is a templated URL specifying which model type `(_type)` user is editing and which specific instance `(_id)` user is editing. If visiting with HTTP GET method, this pages returns a rendered page, which is a form with information of the instance that is being edited. The user can fill different information in this form and use HTTP POST method to send the information to the server. A Python function called `edit()` takes arguments `_type and _id` and gets the right instance from the database. When user submits information from editing page, an HTTP POST request is sent to the server; `edit()` function then handles HTTP Form and abstracts all the data to modify the instance. Relationships are also rebuilt in this process and we build such a rule: an instance can only change what it belongs to, it cannot change what belongs to them. For example, a user can go to Song #1 and change which artist this song belongs to and which albums it belongs to; a user cannot go to Artist page to change what songs this artist owns. This simplifies the task. SQLAlchemy offers great convenience to editing database using Python friendly grammar. See GitHub repo for codes we wrote. When editing is finished, a user is redirected to the instance page. This is done with `redirect` from Python Flask.

For deleting, it is done in the editing process. There is a check box on editing page indicating whether the user wants to delete this instance of the model. HTTP Form will have information called ‘delete’, if it exists, Flask scripts simply remove this instance from the database. When finished, the user is simply redirected to the index page.

For adding, the endpoint is `/add/<string:_type>`, which is a templated URL indicating which type of model is being added to the database. When visited with HTTP GET method, it just returns am an empty form, rendered according to the type. When the user fills the form and uses HTTP POST to send it to the server, a Python function called `add()` is called with argument `_type`. It instantiates a new instance of model `_type` and commits to database. When finished, the user will be redirected to the newly added instance.

On the front-end aspect, we created a button that links to the specific edit/add page. Each of our model instances has an "Edit" button that will allow the user to edit an existing artist, album, song, or tour. The user is also allowed to delete an instance through this page. We decided to restrict the ability to change the name of the models because we felt that it would complicate things and make our database look horrible. At first, we had some trouble figuring out how to integrate React routing with Flask, because initially when we created the button to direct to the /edit page, it would only direct through React instead of going to our edit.html file that we needed to route to. We resolved this issue by using `<a href = { LINK } >` around the button.
...
In order to prevent bots from coming in and altering the database, we implemented Google's reCAPTCHA tool to protect against that. Luckily thanks to a handy Python package, `flask-recaptcha`, this process was relatively simple.

First you import the the package `from flask_recaptcha import ReCaptcha` and then you instantiate a recapthca object; `recaptcha = ReCaptcha(app=app, site_key='', secret_key='')`.

Once that is complete you past the following code into you flask templates where you want the reCAPTCHA to render:

`<script src='//www.google.com/recaptcha/api.js'></script>`
`<div class="g-recaptcha top-buffer" data-sitekey="" data-theme="light" data-type="image" data-size="normal" data-tabindex="0"></div>`

After that, all you have to do to is run this simple code to see if the reCAPTCHA succeeded and handle the outcome appropriately.
